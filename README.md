...
The raw weather dataset originates from the official Hong Kong Observatory Open Data portal page at https://data.gov.hk/en-data/dataset/hk-hko-rss-daily-weather-extract and is locally maintained at `data/daily_temp_2026.csv`. The file contains exactly 744 rows of observations, where each row represents a single hourly micro-climatic recording for all 31 days in August 2026. The values capture the exact timestamp (`YYYY-MM-DD HH:MM:SS`) and the corresponding mean temperature measured in degrees Celsius (°C).
